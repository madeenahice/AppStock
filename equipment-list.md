# รายการจากชีทต้นฉบับ

แหล่งข้อมูล: ต้นฉบับข้อมูล ('ต้นฉบับข้อมูล'!A1:N200)

## Med Stock (17 รายการ)

- CPM 10 mg — จำนวนที่ต้องมี 5
- Dexa 4 mg — จำนวนที่ต้องมี 5
- Diazepam 10 mg — จำนวนที่ต้องมี 1
- NTG 25 mg/5 ml — จำนวนที่ต้องมี 4
- Nicardipine 25mg/25ml — จำนวนที่ต้องมี 1
- Norepineprhine 4 mg/4ml — จำนวนที่ต้องมี 1
- Furosmide 20 mg — จำนวนที่ต้องมี 2
- 50% Glucose 50 ml — จำนวนที่ต้องมี 1
- Beradual Forte  NB — จำนวนที่ต้องมี 5
- Ventolin 2.5 mg NB — จำนวนที่ต้องมี 5
- Pulmicort 1mg/2ml NB — จำนวนที่ต้องมี 5
- Hydrocortisone 100 mg — จำนวนที่ต้องมี 2
- Ondransetron 4 mg/2ml — จำนวนที่ต้องมี 2
- Dimenhydrinate 50 mg — จำนวนที่ต้องมี 1
- Paracetamol 500 mg — จำนวนที่ต้องมี 20
- Chloram — จำนวนที่ต้องมี 1
- Para Syr (120mg/ml) — จำนวนที่ต้องมี 1

## IV Fluid (13 รายการ)

- 0.9% NSS 1000 ml — จำนวนที่ต้องมี 3
- 5% DN/2 1000 ml. — จำนวนที่ต้องมี 2
- 5% DNSS 1000 ml — จำนวนที่ต้องมี 2
- Ringer 1000 ml. — จำนวนที่ต้องมี 1
- Acetar1000 ml — จำนวนที่ต้องมี 2
- 0.9% NSS 500 ml — จำนวนที่ต้องมี 3
- 5% DNSS 500 ml — จำนวนที่ต้องมี 2
- 5% DN/2 500 ml — จำนวนที่ต้องมี 2
- 5% DN/3 500 ml — จำนวนที่ต้องมี 1
- 0.9% NSS 100 ml. — จำนวนที่ต้องมี 5
- 5% DW 100 ml. — จำนวนที่ต้องมี 2
- 10% DN/2 1000 ml. — จำนวนที่ต้องมี 2
- 5% DW 250 ml. — จำนวนที่ต้องมี 4

## Equipment (9 รายการ)

- EKG 12 lead — จำนวนที่ต้องมี 1
- Infusion pump — จำนวนที่ต้องมี 11
- Body guard — จำนวนที่ต้องมี 10
- Ventilator C-1 — จำนวนที่ต้องมี 2
- Ventilator T-1 — จำนวนที่ต้องมี 2
- Defibrillator X-series — จำนวนที่ต้องมี 3
- Syring pump — จำนวนที่ต้องมี 2
- HFNC Comen — จำนวนที่ต้องมี 3
- Ultrasound — จำนวนที่ต้องมี 1

## CPR Box (18 รายการ)

- Adrenaline (1:1000) 1 mg — จำนวนที่ต้องมี 15
- Atropine 0.6 mg — จำนวนที่ต้องมี 6
- Adenosine 6 mg/2ml — จำนวนที่ต้องมี 5
- 10% Calcium gluconate — จำนวนที่ต้องมี 6
- 7.5% NaCHO3 50 ml — จำนวนที่ต้องมี 3
- 50% MgSO4 2 ml — จำนวนที่ต้องมี 2
- 50% Glucose 50 ml — จำนวนที่ต้องมี 1
- Amiodarone 150 mg — จำนวนที่ต้องมี 3
- 2% Lidocaine (Free Preservative) — จำนวนที่ต้องมี 3
- Norepinephrine 4 mg/4ml — จำนวนที่ต้องมี 1
- Dexamethasone 4 mg — จำนวนที่ต้องมี 2
- Chlorpheniramine10mg/1ml — จำนวนที่ต้องมี 1
- Diazepam 10 mg. — จำนวนที่ต้องมี 1
- Dopamine injection 200 mg 10 ml *เฉพาะหน่วยไตเทียม — จำนวนที่ต้องมี 1
- 0.9% NSS 100 ml. — จำนวนที่ต้องมี 1
- D5W 100 ml — จำนวนที่ต้องมี 1
- D5W 250 ml — จำนวนที่ต้องมี 1
- 0.9% NSS 1000 ml. — จำนวนที่ต้องมี 2

## Emer medicine (17 รายการ)

- Adrenaline Injection 1 mg/ml (1:1000) — จำนวนที่ต้องมี 10
- Atropine 0.6 mg 1 ml — จำนวนที่ต้องมี 2
- 10% Calcium gluconate 10 ml — จำนวนที่ต้องมี 1
- Cordarone 150 mg/3ml Inj — จำนวนที่ต้องมี 3
- 50% Glucose 50 ml — จำนวนที่ต้องมี 1
- Dopamex injection 250 mg 10 ml — จำนวนที่ต้องมี 1
- Diazepam 10 mg/2 ml 2 ml — จำนวนที่ต้องมี 1
- 50%Magnesium sulfate 2 ml — จำนวนที่ต้องมี 2
- Adenocor 6 mg 2 ml — จำนวนที่ต้องมี 5
- Hydrocortisone 100 mg 2 ml — จำนวนที่ต้องมี 1
- 7.5% Sodium Bicarbonate Inj 50 ml — จำนวนที่ต้องมี 1
- Lasix 20 mg/2ml — จำนวนที่ต้องมี 2
- Chlorpheniramine10mg/1ml — จำนวนที่ต้องมี 2
- Dexamethazone 4 mg — จำนวนที่ต้องมี 2
- Beradual forte 4 ml — จำนวนที่ต้องมี 2
- Ventolin 2.5 ml — จำนวนที่ต้องมี 2
- 2% Lidocain without Adrenaline (20mg/ml ) — จำนวนที่ต้องมี 3

## Emer cart Adult (48 รายการ)

- Laryngoscope handle + blade — จำนวนที่ต้องมี 1
- Stethoscope — จำนวนที่ต้องมี 1
- กรรไกร — จำนวนที่ต้องมี 1
- ใบ record CPR — จำนวนที่ต้องมี 5
- Magill forceps (Adult) — จำนวนที่ต้องมี 1
- Magill forceps (Ped) — จำนวนที่ต้องมี 1
- Oropharyngeal airwa เขียว — จำนวนที่ต้องมี 1
- Oropharyngeal airwa เหลือง — จำนวนที่ต้องมี 1
- ET-tube No.6.0 — จำนวนที่ต้องมี 2
- ET-tube 6.5 — จำนวนที่ต้องมี 2
- ET-tube 7.0 — จำนวนที่ต้องมี 2
- ET-tube 7.5 — จำนวนที่ต้องมี 2
- ET-tube 8.0 — จำนวนที่ต้องมี 2
- Sterile Stylet no. 10 — จำนวนที่ต้องมี 2
- Sterile Stylet no.12 — จำนวนที่ต้องมี 2
- Sterile Stylet no.14 — จำนวนที่ต้องมี 2
- Suction no.8 — จำนวนที่ต้องมี 2
- Suction no.10 — จำนวนที่ต้องมี 2
- Suction no.12 — จำนวนที่ต้องมี 2
- Suction no.14 — จำนวนที่ต้องมี 2
- Suction no.16 — จำนวนที่ต้องมี 2
- Strap tube/ Sterile gel /Syring 10 ml — จำนวนที่ต้องมี 3
- IV catheter No. 18 — จำนวนที่ต้องมี 3
- IV catheter No. 20 — จำนวนที่ต้องมี 3
- IV catheter No. 22 — จำนวนที่ต้องมี 3
- IV catheter No. 24 — จำนวนที่ต้องมี 3
- Needle No.18 — จำนวนที่ต้องมี 10
- Needle No.20 — จำนวนที่ต้องมี 10
- Needle No.22 — จำนวนที่ต้องมี 10
- Syringe size 3 ml — จำนวนที่ต้องมี 5
- Syringe size 5 ml — จำนวนที่ต้องมี 5
- Syringe size 10 ml — จำนวนที่ต้องมี 5
- Syringe size 20 ml — จำนวนที่ต้องมี 3
- Syringe size 50 ml — จำนวนที่ต้องมี 3
- IV set — จำนวนที่ต้องมี 3
- Extension with T — จำนวนที่ต้องมี 3
- Extension No.18" — จำนวนที่ต้องมี 3
- Tegaderm 6*7 — จำนวนที่ต้องมี 3
- 3-way — จำนวนที่ต้องมี 3
- O2 mask with bag (Adult) — จำนวนที่ต้องมี 2
- Ambu-bag (Adult) — จำนวนที่ต้องมี 1
- Sterile gloves No. 6.5 — จำนวนที่ต้องมี 2
- Sterile gloves No.7 — จำนวนที่ต้องมี 2
- Sterile gloves No. 7.5 — จำนวนที่ต้องมี 2
- CPE Gown — จำนวนที่ต้องมี 2
- Glasses — จำนวนที่ต้องมี 1
- ถังออกซิเจน (กรณีไม่มี Oxygen Pipe line) — จำนวนที่ต้องมี 1
- CPR Board — จำนวนที่ต้องมี 1

## Emer cart PED (60 รายการ)

- Laryngoscope handle — จำนวนที่ต้องมี 1
- blade (Ped) no.00 ตรง — จำนวนที่ต้องมี 1
- blade (Ped) no.0 ตรง — จำนวนที่ต้องมี 1
- blade (Ped) no.1 ตรง — จำนวนที่ต้องมี 1
- blade (Ped) no.00, 0, 1ตรง, — จำนวนที่ต้องมี 1
- blade (Ped) 1 โค้ง — จำนวนที่ต้องมี 1
- blade (Ped) 2 โค้ง — จำนวนที่ต้องมี 1
- Stethoscope — จำนวนที่ต้องมี 1
- กรรไกร — จำนวนที่ต้องมี 1
- Bladeตรง no.0 — จำนวนที่ต้องมี 1
- Bladeตรง no.1 — จำนวนที่ต้องมี 1
- face mask no.0 — จำนวนที่ต้องมี 1
- face mask no.1 — จำนวนที่ต้องมี 1
- Sterlile syring ball no.0 — จำนวนที่ต้องมี 1
- Sterlile syring ball no.1 — จำนวนที่ต้องมี 1
- ปรอทวัดทางทวาร — จำนวนที่ต้องมี 2
- Cord clamp — จำนวนที่ต้องมี 5
- สายวัดความยาว — จำนวนที่ต้องมี 1
- lung test — จำนวนที่ต้องมี 1
- EtCo2 pediatric — จำนวนที่ต้องมี 1
- OG tube no 6 — จำนวนที่ต้องมี 2
- OG tube no 8 — จำนวนที่ต้องมี 2
- หมวกเด็ก — จำนวนที่ต้องมี 1
- Red dot — จำนวนที่ต้องมี 4
- Magill forceps (Ped) — จำนวนที่ต้องมี 1
- Oropharyngeal airway ขาว — จำนวนที่ต้องมี 1
- ET-tube No.2.5 uncuffed — จำนวนที่ต้องมี 2
- ET-tube No.3.0 uncuffed — จำนวนที่ต้องมี 2
- ET-tube No.3.0 with cuff — จำนวนที่ต้องมี 2
- ET-tube No.3.5 with cuff — จำนวนที่ต้องมี 2
- ET-tube 4.0 with cuff — จำนวนที่ต้องมี 2
- ET-tube 4.5 with cuff — จำนวนที่ต้องมี 2
- ET-tube 5.0 with cuff — จำนวนที่ต้องมี 2
- ET-tube 5.5 with cuff — จำนวนที่ต้องมี 2
- Sterile Stylet no.6 — จำนวนที่ต้องมี 1
- Sterile Stylet no.10 — จำนวนที่ต้องมี 1
- Suction no.6 — จำนวนที่ต้องมี 2
- Suction no.8 — จำนวนที่ต้องมี 2
- Suction no.10 — จำนวนที่ต้องมี 2
- Strap tube/ Sterile gel /syring 3 ml — จำนวนที่ต้องมี 3
- Face mask no.0 — จำนวนที่ต้องมี 7
- Face mask no.1 — จำนวนที่ต้องมี 8
- Face mask no.2 — จำนวนที่ต้องมี 5
- O2 mask with bag (Ped) — จำนวนที่ต้องมี 2
- Ambu-bag (Neonatal) — จำนวนที่ต้องมี 1
- Ambu-bag (Ped) — จำนวนที่ต้องมี 1
- Sterile gloves No. 6 — จำนวนที่ต้องมี 2
- Sterile gloves No. 6.5 — จำนวนที่ต้องมี 2
- Sterile gloves No. 7 — จำนวนที่ต้องมี 2
- Sterile gloves No. 7.5 — จำนวนที่ต้องมี 2
- CPE Gown — จำนวนที่ต้องมี 2
- Glasses — จำนวนที่ต้องมี 1
- ถังออกซิเจน (กรณีไม่มี Oxygen Pipe line) — จำนวนที่ต้องมี 1
- CPR Board — จำนวนที่ต้องมี 1
- Oxygen tube (ชั้น 1 Radiant warmmer) — จำนวนที่ต้องมี 2
- 3-piece (ชั้น 1 Radiant warmmer) — จำนวนที่ต้องมี 1
- Tegaderm 4*4 (ชั้น 1 Radiant warmmer) — จำนวนที่ต้องมี 5
- Plastic wrap (ชั้น 2 Radiant warmmer) — จำนวนที่ต้องมี 2
- Thermo trace central (ชั้น 2 Radiant warmmer) — จำนวนที่ต้องมี 3
- Thermo trace peripheral (ชั้น 2 Radiant warmmer) — จำนวนที่ต้องมี 3
